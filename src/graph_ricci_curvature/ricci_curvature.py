...
class RicciCurvature(ABC):
    ...
    def _check_edge_weights(self):
        if not nx.get_edge_attributes(self.G, self.weight_key):
            sys.stderr.write("No edge weights detected, setting edge weights to one with weight_key = weight\n")
    ...
    def _check_directed(self):
        if self.G.is_directed():
            raise NotImplementedError("Directed Graphs are not implemented. Set your graph to undirected with self.G.to_undirected().")
